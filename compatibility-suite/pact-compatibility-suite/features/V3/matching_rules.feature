Feature: V3 era Matching Rules
  ...
  Scenario: Supports a ContentType matcher (negative case)
    ...
    When the request is compared to the expected one
    Then the comparison should NOT be OK
    And the mismatches will contain a mismatch with error "$" -> "Expected binary contents to have content type 'image/jpeg' but inferred contents are 'application/pdf', magic contents are 'application/octet-stream'"
