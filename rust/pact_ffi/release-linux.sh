...
EOM
        ...
        docker run \
            ...
            --rm \
            -v "$TARGET_DIRx86_64-unknown-linux-gnu/release:/scratch" \
            debian \
            /bin/sh -c "$BUILD_SCRIPT"

        gzip_and_sum \
            "$TARGET_DIRx86_64-unknown-linux-gnu/release/libpact_ffi.a" \
...
build_x86_64_musl() {
    ...
    # cargo clean
    install_cross
    RUSTFLAGS="-C target-feature=-crt-static" cross build --target x86_64-unknown-linux-musl "${cargo_flags[@]}"
...
EOM
        ...
        docker run \
            ...
            --rm \
            -v "$TARGET_DIR/aarch64-unknown-linux-gnu/release:/scratch" \
            debian \
            /bin/sh -c "$BUILD_SCRIPT"
        gzip_and_sum \
...
build_aarch64_musl() {
    # cargo clean
    install_cross_latest
    RUSTFLAGS="-C target-feature=-crt-static" cross build --target aarch64-unknown-linux-musl "${cargo_flags[@]}"
